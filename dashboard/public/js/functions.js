...
{
	httpRequest = new XMLHttpRequest();
	httpRequest.open('GET', 'FastSync.php', true);
	httpRequest.send();
	setTimeout(function() {
...
function ResetPasswordPrompt()
{
	...
	passwordbox.id = "password_prompt_overlay";
	passwordbox.innerHTML = '<div id="password_prompt"><h2>Reset Password</h2><div id="inputs"><label for="password">Password:</label><input type="password" id="password" name="password"><br /><label for="confirm_password">Confirm Password:</label><input type="password" name="confirm_password" id="confirm_password" /></div><button id="reset_password_button" onclick="ResetPassword()"; type="button">Submit</button></div>';
...
function SetWireless()
{
	...
	httpRequest.onreadystatechange = function() {
		if(httpRequest.readyState == 4 && httpRequest.status == 200) {
			alert(httpRequest.responseText);
			var wireless_box = document.getElementById("wireless_prompt_overlay");
			wireless_box.remove();
		}
	}
